...
# Folders :
# TestingData : SquareRegisteredTestingData
# TrainingData : SquareRegisteredTrainingData

root = os.getcwd()

test = "SquareRegisteredTestingData"
train = "SquareRegisteredTrainingData"
...
def load_images(folder):
    ...
    temp_data = np.ndarray(shape=(image_size, image_size), dtype=np.float)
    print(folder)
    num_images = 0
    for Image in image_files:
        image_file = os.path.join(folder, Image)
        try:
            dicom_image = dicom.read_file(image_file)
            temp_data = dicom_image.pixel_array

            image_data = (temp_data.astype(float) - pixel_depth / 2) / pixel_depth
...
def merge_datasets(pickle_files, train_size, valid_size=0):
    ...
    # # let's shuffle the images to have random validation and training set
